#!/bin/bash
# Assemble the QEMU VM disk image (qcow2) from rootfs.tar
# (run as root in WSL2 / any Linux with loop devices).
set -ex
...
rm -f disk.raw
truncate -s 16G disk.raw
parted -s disk.raw mklabel gpt mkpart ESP fat32 1MiB 513MiB set 1 esp on mkpart root ext4 513MiB 100%

LOOP=$(losetup --find --show -P disk.raw)
sleep 1
...
tar --numeric-owner --xattrs -xpf "$OUT/rootfs.tar" -C /mnt/holo
...
# keep /boot/Image fresh when the kernel is updated inside the VM
...
# console=hvc0: this kernel has no PL011 driver, so the console must be
# virtio (see README troubleshooting #1). The fallback initramfs is used
# because autodetect inside a container drops virtio modules.
cat > /mnt/holo/boot/loader/entries/holo.conf <<EOF
title   Holo core aarch64 (preview)
linux   /Image
initrd  /initramfs-linux-fallback.img
options root=PARTUUID=$ROOTPARTUUID rw console=tty1 console=hvc0
EOF
...
qemu-img convert -O qcow2 disk.raw "$OUT/holo-arm.qcow2"
qemu-img info "$OUT/holo-arm.qcow2"
rm -f disk.raw
echo ASSEMBLY_DONE
